...
export interface MoveDef {
  ...
  range?: number;
  cooldown?: number;
  duration?: number; // for temporary effects in ticks
  statusEffect?: Omit<StatusEffect, 'remainingTicks'>; // applied on use
  attackProfile?: AttackProfile; // frame data for combat moves
  // For boosts:
  statChanges?: Partial<{
    maxHp: number;
...
export const MOVE_CATALOG: Record<string, MoveDef> = {
  basic_attack: {
    id: 'basic_attack',
    name: 'Ataque Básico',
    cost: 0,
...
export function getAvailableMoves(agent: AgentState): string[] {
  // Every agent always has basic_attack
  const moves = ['basic_attack', ...agent.moves];
  return moves;
}

export function isMoveOffCooldown(agent: AgentState, moveId: string): boolean {
  return (agent.cooldowns[moveId] ?? 0) <= 0;
}

export function getMoveDef(moveId: string): MoveDef | undefined {
