import { RoomPlayer, Stats } from '@shared/index';
...
export function getAvailableItems(player: RoomPlayer): string[] {
  ...
  return Object.keys(MOVE_CATALOG).filter((id) => {
    if (id === 'basic_attack') return false; // not purchasable
...
export function purchaseItem(player: RoomPlayer, itemId: string): PurchaseResult {
...
function applyStatChanges(stats: Stats, changes: Partial<Stats>): void {
  if (changes.maxHp !== undefined) stats.maxHp += changes.maxHp;
  if (changes.movementSpeed !== undefined) stats.movementSpeed += changes.movementSpeed;
  if (changes.attackDamage !== undefined) stats.attackDamage += changes.attackDamage;
}
