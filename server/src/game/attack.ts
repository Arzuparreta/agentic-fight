...
export function startAttack(
  ...
  moveId: string,
  tick: number
): { success: boolean; events: SimEvent[] } {
 ...
 */
export function tickAttackState(
  agent: AgentState,
  opponent: AgentState,
  state: GameState
): SimEvent[] {
  ...
  if (atk.phase === 'idle') return events;
...
function resetAttackState(agent: AgentState): void {
  agent.attackState = {
    phase: 'idle',
    moveId: null,
    ticksInPhase: 0,
 ...
 */
function resolveHit(attacker: AgentState, target: AgentState, baseDamage: number, tick: number): number {
 ...
 */
export function canInitiateAttack(agent: AgentState): boolean {
 ...
 */
export function isVulnerable(agent: AgentState): boolean {
 ...
 */
export function isWindingUp(agent: AgentState): boolean {
