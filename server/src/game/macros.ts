...
import { startAttack, canInitiateAttack, isWindingUp, isVulnerable } from './attack';
import { tickRand } from './rng';

/* ─── Types ─── */

export interface MacroResult {
  inputAngle: number | null;   // angle to accelerate toward; null = no movement input
  speedMult: number;           // multiplier to max speed
  moveId: string | null;       // attack move to initiate
  dodgeDir: string | null;     // dodge direction to trigger
  done: boolean;               // true if this macro considers itself complete
  reasoning: string;
}

/* ─── Interrupt Predicates ─── */

const INTERRUPT_PREDICATES: Record<string, (agent: AgentState, opponent: AgentState, state: GameState) => boolean> = {
...
export function initSequenceExecution(seq: ActionSequence) {
  return {
    sequence: seq,
...
export function tickSequenceExecution(
  exec: NonNullable<AgentState['sequenceExecution']>,
  agent: AgentState,
  opponent: AgentState,
  state: GameState
): { result: MacroResult; advanced: boolean; events: { type: string; payload: Record<string, unknown> }[] } {
...
function macroApproach(agent: AgentState, opponent: AgentState, ticks: number): MacroResult {
...
function macroCircle(agent: AgentState, opponent: AgentState, direction: 1 | -1, ticks: number): MacroResult {
...
function macroFeintApproach(agent: AgentState, opponent: AgentState, state: GameState, ticks: number): MacroResult {
...
function macroBait(agent: AgentState, opponent: AgentState, state: GameState, ticks: number): MacroResult {
  const dist = distanceBetweenAgents(agent, opponent);
  const oppMaxRange = getBestAttackRange(opponent);
...
function macroDodge(agent: AgentState, opponent: AgentState, hint?: string): MacroResult {
...
function macroRetreat(agent: AgentState, opponent: AgentState, ticks: number): MacroResult {
...
function macroShieldUp(agent: AgentState, opponent: AgentState): MacroResult {
  if (!canInitiateAttack(agent)) {
...
function macroWait(): MacroResult {
  return { inputAngle: null, speedMult: 1, moveId: null, dodgeDir: null, done: false, reasoning: 'Waiting for opening' };
}

function macroKite(agent: AgentState, opponent: AgentState, ticks: number): MacroResult {
  const dist = distanceBetweenAgents(agent, opponent);
  const toOpp = angleToTarget(agent, opponent.physics.position);
...
function macroPunish(agent: AgentState, opponent: AgentState, state: GameState, ticks: number): MacroResult {
...
function macroDodgeAndCounter(agent: AgentState, opponent: AgentState, state: GameState, ticks: number): MacroResult {
...
function macroRushdown(agent: AgentState, opponent: AgentState, ticks: number): MacroResult {
  const dist = distanceBetweenAgents(agent, opponent);
...
function getBestAttackRange(agent: AgentState): number {
...
function getBestAvailableAttackMove(agent: AgentState, opponent: AgentState): string | null {
  const dist = distanceBetweenAgents(agent, opponent);
  ...
  for (const m of getAvailableMoves(agent)) {
    const d = getMoveDef(m);
    if (!d || !d.damage || d.damage <= 0) continue;
    if (!isMoveOffCooldown(agent, m)) continue;
    if (d.range && dist > d.range) continue;
...
function parseDirectionHint(hint: string, baseAngle: number): number {
  const h = hint.toLowerCase();
  if (h.includes('back')) {
    if (h.includes('left')) return baseAngle + Math.PI * 0.75;
    if (h.includes('right')) return baseAngle - Math.PI * 0.75;
    return baseAngle + Math.PI;
  }
  if (h.includes('left')) return baseAngle + Math.PI / 2;
  if (h.includes('right')) return baseAngle - Math.PI / 2;
  if (h.includes('forward')) return baseAngle;
