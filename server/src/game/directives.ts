...
const PATTERN_KEYWORDS: Record<string, MovementPattern[]> = {
...
const REACTION_KEYWORDS: Record<string, Partial<TacticalReactions>> = {
...
const RANGE_KEYWORDS: Record<string, number> = {
  'close range': 10,
  'melee': 15,
  'short range': 20,
  'mid range': 50,
...
export function parseDirectives(directives: string[]): ParsedDirective[] {
  const parsed: ParsedDirective[] = [];

  for (const directive of directives) {
    const lower = directive.toLowerCase();
    const pd: ParsedDirective = { description: directive };
...
export function mergeDirectivesIntoPlan(
  plan: TacticalPlan,
