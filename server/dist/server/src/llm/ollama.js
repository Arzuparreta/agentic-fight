import { getAvailableMoves, getMoveDef, VALID_MACRO_ACTIONS, } from '@shared/index';
import { analyzeOpponentTendencies } from '../game/behavior';
function defaultSequenceFailure(reason) {
    return {
        strategy: reason,
...
function formatAvailableMoves(agent) {
...
function buildSequencePrompt(agent, opponent, state, playstyleProfile, characterDescription, actionHistory, opponentHistory, tendencies, roundSummaries, errorContext) {
    ...
    if (Array.isArray(parsed.actions)) {
        for (const a of parsed.actions) {
            if (!a || typeof a !== 'object')
                continue;
            const macro = VALID_MACRO_ACTIONS.includes(a.macro) ? a.macro : 'approach';
...
async function generateSequenceOnce(prompt, agentId) {
    try {
...
export async function getSequenceWithTrace(agent, opponent, state, playstyleMemory, characterDescription, actionHistory, opponentHistory, roundSummaries, errorContext) {
    const t0 = performance.now();
    let playstyleProfile = null;
    if (playstyleMemory) {
        try {
    ...
    const prompt = buildSequencePrompt(agent, opponent, state, playstyleProfile, characterDescription, actionHistory, opponentHistory, tendencies, roundSummaries, errorContext);
    ...
    try {
        ...
        const durationMs = performance.now() - t0;
        console.log(`[LLM] Sequence for ${agent.id}: ${sequence.actions.map((a) => a.macro).join(' -> ')}`);
