import { getAvailableMoves, getMoveDef, MOVE_CATALOG, VALID_MOVEMENT_PATTERNS, VALID_REACTION_OPTIONS, } from '@shared/index';
import { analyzeOpponentTendencies } from '../game/behavior';
function defaultTacticalPlanFailure(reason) {
    return {
        strategy: reason,
...
function formatActionHistory(history, maxEntries = 10) {
    if (history.length === 0)
...
function formatTendencies(tendencies) {
...
function buildTacticalPrompt(agent, opponent, state, playstyleProfile, characterDescription, actionHistory, opponentHistory, tendencies, roundSummaries, errorContext) {
...
function validateTacticalPlan(parsed) {
    const defaultPlan = {
...
function buildMinimalRetryPrompt(agent, opponent, state) {
...
async function generateTacticalPlanOnce(prompt, agentId) {
    try {
...
export async function getTacticalPlanWithTrace(agent, opponent, state, playstyleMemory, characterDescription, actionHistory, opponentHistory, roundSummaries, errorContext) {
    ...
    const prompt = buildTacticalPrompt(agent, opponent, state, playstyleProfile, characterDescription, actionHistory, opponentHistory, tendencies, roundSummaries, errorContext);
...
export async function getTacticalPlan(agent, opponent, state, playstyleMemory, characterDescription, actionHistory, opponentHistory, roundSummaries, errorContext) {
    const r = await getTacticalPlanWithTrace(agent, opponent, state, playstyleMemory, characterDescription, actionHistory, opponentHistory, roundSummaries, errorContext);
    return r.plan;
}
...
export async function getAgentPlan(agent, opponent, state, playstyleMemory, characterDescription, errorContext) {
    const tactical = await getTacticalPlan(agent, opponent, state, playstyleMemory, characterDescription, [], [], [], errorContext);
    const planMap = {
...
export async function generateRoundSummary(agentName, characterDescription, eventLog, won, agentId, opponentId) {
