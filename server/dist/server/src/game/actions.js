import { getMoveDef, ARENA, AGENT_HITBOX_RADIUS, DODGE_DISTANCE, DODGE_COOLDOWN, DODGE_INVINCIBILITY_TICKS, DIAGONAL_SPEED_FACTOR, } from '@shared/index';
...
function calculateDamage(agent, baseDamage) {
...
function applyDamage(target, rawDamage, currentTick) {
...
function applyMovement(agent, action) {
...
function applyDodge(agent, action, currentTick) {
    const oldPos = { x: agent.position.x, y: agent.position.y };
    let dx = 0;
    let dy = 0;
    ...
    if (action.includes('right'))
        dx += dist;
    if (action.includes('up') && !action.includes('left') && !action.includes('right'))
        dy -= dist;
    if (action.includes('down') && !action.includes('left') && !action.includes('right'))
        dy += dist;
    if (action === 'dodge_up_left') {
        dx = -dist * DIAGONAL_SPEED_FACTOR;
        dy = -dist * DIAGONAL_SPEED_FACTOR;
    }
    if (action === 'dodge_up_right') {
        dx = dist * DIAGONAL_SPEED_FACTOR;
        dy = -dist * DIAGONAL_SPEED_FACTOR;
    }
    if (action === 'dodge_down_left') {
        dx = -dist * DIAGONAL_SPEED_FACTOR;
        dy = dist * DIAGONAL_SPEED_FACTOR;
    }
    if (action === 'dodge_down_right') {
        dx = dist * DIAGONAL_SPEED_FACTOR;
        dy = dist * DIAGONAL_SPEED_FACTOR;
    }
...
function resolveCollision(agents) {
...
function updateFacing(agent, opponent) {
...
export function applyActions(state, actions) {
    ...
    for (const id of agentIds) {
        ...
        if (agent.status === 'dead' || opponent.status === 'dead')
            continue;
        const act = actions[id].action;
        const def = getMoveDef(act);
        if (!def || def.type !== 'move')
            continue;
        if (isDodgeAction(act))
            continue;
