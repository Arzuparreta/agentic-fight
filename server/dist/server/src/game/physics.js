import { ARENA, AGENT_HITBOX_RADIUS, ACCELERATION, FRICTION, TURN_SPEED, DODGE_BURST_SPEED, ATTACK_FACING_TOLERANCE, getMoveDef, } from '@shared/index';
/* ─── Helpers ─── */
export function createDefaultPhysics(stats) {
    return {
        position: { x: 0, y: 0 },
        velocity: { x: 0, y: 0 },
 ...
 */
export function applyMovementInput(agent, inputAngle, speedMult = 1.0) {
 ...
 */
export function zeroVelocity(agent) {
    agent.physics.velocity = { x: 0, y: 0 };
}
/**
 * Apply friction and update position.
 */
export function stepPhysics(agent) {
    const phys = agent.physics;
    phys.velocity.x *= phys.friction;
    phys.velocity.y *= phys.friction;
    // Stop micro-drifting
    if (Math.abs(phys.velocity.x) < 0.05)
        phys.velocity.x = 0;
    if (Math.abs(phys.velocity.y) < 0.05)
        phys.velocity.y = 0;
 ...
 */
export function clampToArena(agent) {
    const phys = agent.physics;
 ...
 */
export function resolveAgentCollision(a, b) {
 ...
 */
export function turnToward(agent, targetAngle) {
    const phys = agent.physics;
 ...
 */
export function snapFacing(agent, angle) {
 ...
 */
export function isFacingTarget(agent, target) {
 ...
 */
export function angleToTarget(agent, target) {
    const dx = target.x - agent.physics.position.x;
    const dy = target.y - agent.physics.position.y;
 ...
 */
export function distanceBetweenAgents(a, b) {
    const dx = b.physics.position.x - a.physics.position.x;
    const dy = b.physics.position.y - a.physics.position.y;
 ...
 */
export function applyDodgeBurst(agent, direction) {
    const phys = agent.physics;
    let dx = 0;
    let dy = 0;
    ...
    if (direction.includes('right'))
        dx += 1;
    if (direction.includes('up') && !direction.includes('left') && !direction.includes('right'))
        dy -= 1;
    if (direction.includes('down') && !direction.includes('left') && !direction.includes('right'))
        dy += 1;
    if (direction === 'dodge_up_left') {
        dx = -1;
        dy = -1;
    }
    if (direction === 'dodge_up_right') {
        dx = 1;
        dy = -1;
    }
    if (direction === 'dodge_down_left') {
        dx = -1;
        dy = 1;
    }
    if (direction === 'dodge_down_right') {
        dx = 1;
        dy = 1;
    }
 ...
 */
export function getCurrentSpeedMultiplier(agent) {
    ...
    if (atk.phase === 'windup')
        return prof.windupMoveSpeedMult;
    if (atk.phase === 'recovery' || atk.phase === 'whiff')
        return prof.recoveryMoveSpeedMult;
    return 1.0; // active phase — full speed (you commit forward with momentum)
}
/* ─── Position helpers ─── */
export function getPosition(agent) {
    return agent.physics.position;
}
export function setPosition(agent, pos) {
