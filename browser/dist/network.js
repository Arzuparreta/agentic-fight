...
function startPlayback(eventLog, _roundResult) {
    if (!renderer)
        return;
    const ui = document.getElementById('ui');
    ui.innerHTML = '';
    ...
    for (const id of agentIds) {
        let position = { x: ARENA.width * 0.25, y: ARENA.height * 0.5 };
        let hp = 150;
        let maxHp = 150;
        ...
        for (const ev of eventLog) {
            if (ev.agentId === id && ev.type === 'move' && ev.payload.newPosition !== undefined) {
                const pos = ev.payload.newPosition;
        ...
        for (const ev of eventLog) {
            if (ev.agentId === id && ev.type === 'hit') {
                const remaining = ev.payload.hpRemaining;
    ...
    playback = new PlaybackController(eventLog, (tick, events) => {
