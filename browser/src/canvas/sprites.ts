...
// Conquistador sprite
const CONQUISTADOR_SPRITE: SpriteDef = {
  width: 12,
  height: 16,
  pixels: [
    '..GGGGGGGG..',
...
export function getSpriteForCharacter(description: string): SpriteDef {
  const lower = description.toLowerCase();
  if (lower.includes('knight') || lower.includes('caballero')) return CHARACTER_SPRITES.knight;
  if (lower.includes('moor') || lower.includes('mor')) return CHARACTER_SPRITES.moor;
  if (lower.includes('friar') || lower.includes('fraile') || lower.includes('inquisitor')) return CHARACTER_SPRITES.friar;
  if (lower.includes('conquistador')) return CHARACTER_SPRITES.conquistador;
