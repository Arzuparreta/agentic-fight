let onNewMessages = null;
export function renderCoachingScreen(container, roomId, playerId, socket, existingMessages) {
    const messages = [];
    container.innerHTML = `
  ...
  `;
    ...
    onNewMessages = (newMessages) => {
        const unseen = newMessages.filter((m) => !messages.some((existing) => existing.sender === m.sender && existing.content === m.content));
...
export function updateCoachingMessages(newMessages) {
    if (onNewMessages) {
