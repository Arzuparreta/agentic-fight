...
export function renderShopScreen(
  ...
  const itemsList = container.querySelector('#items-list') as HTMLDivElement;
  const moneyDisplay = container.querySelector('#money-display') as HTMLParagraphElement;
  const purchaseMsg = container.querySelector('#purchase-msg') as HTMLDivElement;
  const readyBtn = container.querySelector('#shop-ready-btn') as HTMLButtonElement;

  let currentMoney = 0;
  let ownedItems = new Set<string>();
  ...
  window.addEventListener('economy-update' as any, ((e: CustomEvent) => {
    const data = e.detail;
  ...
  window.addEventListener('purchase-result' as any, ((e: CustomEvent) => {
